...
# It assumes that the working directory contains these files: an OpenSearch tarball (opensearch.tgz), log4j2.properties, opensearch.yml, opensearch-docker-entrypoint.sh, opensearch-onetime-setup.sh, custom-opensearch.yml, change-password.sh
# Build arguments:
...
#   UID: Optional. Specify the opensearch userid. Defaults to 1999.
#   GID: Optional. Specify the opensearch groupid. Defaults to 1999.
...
RUN ls -l $TEMP_DIR && \
    ...
    $TEMP_DIR/opensearch-patch-image.sh $OPENSEARCH_HOME && \
    ...
    cp -v $TEMP_DIR/custom-opensearch.yml $TEMP_DIR/change-password.sh $OPENSEARCH_PATH_CONF/ && \
